...
|---|---|
| 문서번호 | IAPQB-STD-001 |
| 버전 | v1.0 (공개초안 · Public Comment Draft) |
...
| v0.9에서 바뀐 것 | 등급 체계를 검정 4등급에 맞춰 재정렬, **산업 모듈 층(제5장) 신설**, 첫 모듈 M1 Physical AI 수록 |
...
**산업 모듈**은 그 코어가 특정 산업에서 어떤 모습이 되는지를 규정한다(제5장). 모듈은 새로운 도메인을 만들지 않는다.
...
같은 D5(검증)를 요구해도 산업마다 **틀렸을 때 부서지는 것**이 다르다. 사무 문서에서 인공지능이 틀리면 문서가 틀린다. 물리적으로 동작하는 기계에서 틀리면 사람이 다친다. 요구되는 검증의 강도와 그 증거의 형태가 같을 수 없다.

그래서 본 표준은 코어를 산업별로 쪼개지 않고, **코어는 하나로 두되 그 위에 산업 모듈을 얹는다.** 코어를 산업별로 쪼개면 자격이 산업마다 갈라져 서로 비교할 수 없게 된다.

### 5.2 모듈의 원칙

**모듈은 새 도메인을 만들지 않는다.** D1~D7 각각이 그 산업에서 어떤 모습인지를 규정할 뿐이다.

**모듈은 코어를 대체하지 않고 얹는다.** 모듈의 전제는 같은 등급의 코어 취득이다.

**모듈은 「무엇이 틀리면 무엇이 부서지는가」를 먼저 정의한다.** 그것이 그 산업에서 요구되는 검증 강도를 정한다.

**모듈은 코어보다 빨리 낡는다.** 코어는 연 1회 정기 개정하고, 모듈은 필요할 때마다 개정한다.

### 5.3 M1 — Physical AI (제조·설비·무인이동체)

**정의.** 인공지능이 로봇·무인이동체·자동화 설비·검사 장비에 탑재되어 사람·설비·제품에 **직접 작용**하는 영역.

**왜 이것이 첫 모듈인가.** 2024년 국내 등록 인공지능 관련 민간자격 107개를 전수 확인한 결과, 로봇 4건·자율주행 1건·사물인터넷 2건이 전부였고 **제조·임베디드·스마트팩토리·머신비전·센서·품질·모빌리티는 0건**이었다. 인공지능이 화면 밖으로 나오는 영역의 실무역량을 다루는 검정 기준이 사실상 비어 있다.

**코어의 각 도메인이 이 모듈에서 어떻게 달라지는가.**

| 도메인 | Physical AI에서의 형태 |
|---|---|
| D1 | 언어 생성이 아니라 **지각–판단–구동 루프**가 대상이다. 실시간 제약과 물리 제약이 있고, 시뮬레이션 성능과 실제 성능의 격차(sim-to-real)를 전제로 다룬다 |
| D2 | 지시가 자연어가 아니라 **작업 명세와 동작 허용 범위**다. 무엇을 하라보다 **무엇을 하지 마라**를 먼저 적는다 |
| D3 | 자료가 문서가 아니라 **센서 시계열과 영상**이다. 결측·드리프트와 조명·진동·온도 같은 환경 변동이 품질 변수가 된다 |
| D4 | 실패가 **정지가 아니라 동작**일 수 있다. 인터록, 비상정지, 안전 정지 범주를 설계에 포함한다 |
| D5 | 검증 대상이 평균 성능이 아니라 **최악 조건에서의 거동**이다. **정확도 99%는 이 영역에서 합격 근거가 되지 못한다.** 실패 모드를 먼저 열거하고 각각을 재현해 본다 |
| D6 | 개인정보·저작권 위에 **기계 안전과 제품 책임**이 얹힌다. 규제 산업이면 밸리데이션 문서 체계와의 접속점을 정의한다 |
| D7 | 지표가 시간 절감만이 아니라 **가동률·불량률·재작업률**이며, **반드시 무재해 여부와 함께** 보고한다. 사고를 대가로 얻은 생산성은 성과로 계산하지 않는다 |

**적용 대상 직무 예시.** 제조 현장의 공정 자동화와 인공지능 품질검사, 물류·창고 로봇 운영, 무인이동체 운용, 설비 예지보전.

### 5.4 모듈 로드맵

M2는 규제 문서 영역(의약품 제조·품질관리, 임상, 금융)을, M3은 공공·행정을 예정한다. 다만 **M1 외에는 착수하지 않는다.** 쓰이지 않는 표준을 늘리는 것은 기준을 만드는 기관이 해서는 안 되는 일이다.

### 5.5 모듈의 검정상 취급 (고지)

현재 등록을 신청한 자격의 종목은 **공통 코어만**을 대상으로 한다. 모듈은 **표준으로 먼저 공개**하고, 검정에 반영하는 것은 등록이 완료된 뒤 별도의 절차를 거친다. 본 절의 내용을 근거로 어떠한 자격도 지금 발급하지 않는다.
...
공통 코어는 연 1회 정기 개정하고, 산업 모듈은 필요할 때마다 개정한다. 모든 개정은 공개 의견수렴을 거치며, 버전·개정 사유·의견 반영 결과를 문서 이력으로 공개한다. 구 버전 기준으로 취득한 자격의 효력과 갱신 요건은 IAPQB-GOV-002에서 정한다.
